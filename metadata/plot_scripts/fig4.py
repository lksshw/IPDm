...
# unicelluar/multicellular cooperability as a function of memory size
...
fig, ax = plt.subplots(2, 2, figsize=(11, 7), sharey=False)
    ...
    for n, i in enumerate(mem_sizes):
        ...
        for r in range(n_runs):
            dat = load(os.path.join(
                mem_path, f"cooperability-vs-time_merge-run{r}.pkl"))
        ...
        u_mean = chunkProcess(u_mean)*100
        u_std = chunkProcess(u_std)*100

        m_mean = chunkProcess(m_mean)*100
        m_std = chunkProcess(m_std)*100
        ...
        ax[row][col].fill_between(list(range(len(u_mean))), np.clip(
            u_mean-mult_fact*u_std, a_min=0.0, a_max=None), u_mean+mult_fact*u_std, alpha=alpha, color="#04030F")

        if (len(pos)):
            pos_idx = list(pos)[0] + 1
            # print(pos_idx)
        ...
        ax[row][col].fill_between(list(range(len(m_mean))), np.clip(
            m_mean-mult_fact*m_std, a_min=0.0, a_max=None), m_mean+mult_fact*m_std, alpha=alpha, color="#F44708")
